...
with zipfile.ZipFile(OUT_TRAIN, 'w') as img_out,\
 zipfile.ZipFile(OUT_MASKS, 'w') as mask_out:
    for name in tqdm(names):
        ...
        concatenated_img = cv2.imencode('.png',concatenated_img)[1]
        img_out.writestr(f'{name}.png', concatenated_img)
        concatenated_mask = cv2.imencode('.png', concatenated_mask)[1]
        mask_out.writestr(f'{name}.png', concatenated_mask)

    img_out.extractall('../input/prostate-cancer-grade-assessment/iafoss')
    mask_out.extractall('../input/prostate-cancer-grade-assessment/iafoss_mask')
